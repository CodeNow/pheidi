 ...
 * @return {Boolean} true if instance should be notified
 */
exports.shouldInstanceBeNotified = (instance) => {
  // do nothing for test instance
  if (instance.isTesting) {
    return false
  }
  const acv = exports.getACVForInstance(instance)
...
exports.getPushInfoForInstance = (instance) => {
  const cv = keypather.get(instance, 'contextVersion') || {}
  const acv = cv.appCodeVersions.find((a) => {
    return !a.additionalRepo
  })
  const state = exports.utils.instanceState(instance)
