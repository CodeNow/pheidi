...
module.exports = class SendGridModel {
  ...
  constructor () {
    ...
    var necessaryKeys = [
      'SENDGRID_KEY',
      'SENDGRID_DOCK_CREATED_TEMPLATE',
      'SENDGRID_PAYMENT_METHOD_ADDED_TEMPLATE',
      'SENDGRID_PAYMENT_METHOD_REMOVED_TEMPLATE',
      'SENDGRID_DOCK_CREATED_TEMPLATE',
      'SENDGRID_SENDER_NAME',
      'SENDGRID_SENDER_EMAIL'
    ]
    var checkEnvExists = x => !process.env[x]
    if (necessaryKeys.some(checkEnvExists)) {
      var missingKeys = necessaryKeys.filter(checkEnvExists).join(', ')
      error = new Error('SENDGRID: Missing the following keys: ${missingKeys}')
