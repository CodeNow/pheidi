...
module.exports = class SendGridModel {
  ...
  paymentMethodAdded (organization, paymentMethodOwnerEmail) {
    const log = logger.child({
      method: 'paymentMethodAdded',
      org: organization.login,
      paymentMethodOwnerEmail: paymentMethodOwnerEmail
    })
    log.info('paymentMethodAdded called')
    const defaultOpts = SendGridModel._getDefaultEmailOptions(PAYMENT_METHOD_ADDED)
    return this.sendEmail(Object.assign(defaultOpts, {
  ...
  paymentMethodRemoved (organization, paymentMethodOwnerEmail) {
    const log = logger.child({
      method: 'paymentMethodRemoved',
      org: organization.login,
      paymentMethodOwnerEmail: paymentMethodOwnerEmail
    })
    log.info('paymentMethodRemoved called')
    const defaultOpts = SendGridModel._getDefaultEmailOptions(PAYMENT_METHOD_REMOVED)
    return this.sendEmail(Object.assign(defaultOpts, {
      email: paymentMethodOwnerEmail,
