...
module.exports = class SendGridModel {
   ...
   * @returns {Promise} resolves when email request is done
   */
  sendingNotificationEmail (organizationName, toEmail, options, templateId, extraSubstitutions) {
    log.info({
      orgName: organizationName,
      toEmail: toEmail
  ...
  trialEnding (organizationName, emailsForAllMembersOfOrganization) {
    log.info({
      orgName: organizationName,
      emailsForAllMembersOfOrganization: emailsForAllMembersOfOrganization
    }, 'trialEnding called')
    return this.sendBillingEmail(
      organizationName,
 ...
 /**
   ...
   * @returns {Promise} resolves when email request is done
   */
  welcomeEmailForOrganization (organizationName, organizationCreatorEmail) {
    log.info({
      orgName: organizationName,
    ...
    return this.sendingNotificationEmail(
      ...
      organizationCreatorEmail,
      emailCopy.BILLING_ERROR_ALL,
      process.env.SENDGRID_WELCOME_EMAIL_TEMPLATE
    )
  }
   ...
   * @returns {Promise} resolves when email request is done
   */
  userAddedToOrganization (organizationName, emailForNewlyAddedUser) {
    log.info({
      orgName: organizationName,
