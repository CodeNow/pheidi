 ...
 * @type {Hermes}
 */
const internalClient = new Hermes({
  name: process.env.APP_NAME,
  hostname: process.env.RABBITMQ_HOSTNAME,
  port: process.env.RABBITMQ_PORT,
  username: process.env.RABBITMQ_USERNAME,
...
/**
 * Handles errors for the hermes publisher.
 * @param {Error} The error to handle.
 */
publisher.on('error', (err) => {
  log.error({ err: err }, 'Hermes publisher encountered an error')
  ErrorCat.report(err)
})
...
module.exports.publisher = Promise.promisifyAll(publisher)
...
module.exports.publishGitHubBotNotify = function (data) {
  publisher.publish('github.bot.notify', data)
}
