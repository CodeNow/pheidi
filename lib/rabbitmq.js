...
}).required().label('Organization.integration.prbot.enabled')

const prBotDisabledSchema = Joi.object({
  organizationId: Joi.number().required()
 ...
 * @param {Object} data job payload
 */
module.exports.publishPrBotDisabled = function (data) {
  log.info(data, 'publish organization.integration.prbot.disabled job')
  return publisher.publishEvent('organization.integration.prbot.disabled', data)
}
