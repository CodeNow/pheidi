    ...
    .spread((org, user) => {
      ...
      return orion.users.find({ email })
        ...
        .then((user) => {
          let messageBody
          if (org.isInTrial) {
            messageBody = 'Hey ' + username + ',<br><br>' +
            ...
            messageBody = 'Hey ' + username + ',<br><br>' + 'Your infrastructure is ready, the <b>' +
              org.name + '</b> environment is good to go.<br><br><a href="https://app.runnable.io/' + org.name +
              '">Go to Runnable.</a><br><br>' + 'Your best friend,<br>Praful Rana<br>Customer Success @ Runnable'
          }
          let message = {
            message_type: 'email',
