          ...
          .spread(function validateModels (instance, cv) {
            ...
            return Promise.props({
              ...
              if (pushInfo && pushInfo.commit) {
                const gitHubDeploy = new GitHubDeploy(accessToken)
              ...
              if (pushInfo && process.env.PR_BOT_WHITELIST.indexOf(instance.owner.github) > -1) {
                const githubBot = new GitHubBot()
                githubBot.notifyOnAutoDeploy(pushInfo, instance, function (err) {
                  if (err) {
