...
module.exports.task = (job) => {
  ...
  const repo = job.payload.pull_request.head.repo.name
  ...
  return mongoClient.findInstancesAsync(query)
    .then((instances) => {
      instances = instances || []
      log.info({ instances: instances }, 'instances instances')
      instances.forEach((instance) => {
        if (!utils.shouldInstanceBeNotified(instance)) {
          log.trace('Instance should not be notified', { instance })
          return
        }
        const pushInfo = utils.getPushInfoForInstance(job.instance)
        if (!pushInfo) {
          log.trace('Instance should not be notified. Instance has no state', { instance })
          return
        }
