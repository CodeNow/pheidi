...
module.exports.task = (job) => {
  ...
  return Promise
      ...
      return mongoClient.findOneContextVersionAsync({_id: new ObjectID(cvId)})
        .then((contextVersion) => {
          ...
          return mongoClient.findInstancesAsync({
            'contextVersion._id': contextVersion._id,
            isTesting: true
          })
        })
    })
    ...
    .each((instance) => {
      ...
      log.trace({'instance.contextVersion.build.message': instance.contextVersion.build.message, 'isTestReporter': instance.isTestReporter}, 'these are test instance properties')
      if (instance.contextVersion.build.message === 'Initial Cluster Creation' && !instance.isTestReporter) {
        throw new WorkerStopError('Instance is in a test cluster, but is not a reporter', {}, { level: 'info' })
      }
