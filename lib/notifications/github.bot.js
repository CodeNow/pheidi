    ...
    this.github.findCommentByUser(gitInfo.repo, gitInfo.number,
      ...
      (err, comment) => {
        ...
        const messageId = hash({
          repo: gitInfo.repo,
          number: gitInfo.number,
          message: newMessage
        })
        const tracked = tracker.get(messageId)
        if (tracked) {
          log.trace('we just send same comment not long time ago')
          return cb(null)
        }
        ...
        this.github.updateComment(gitInfo.repo, comment.id, newMessage, function (err, resp) {
          if (err) {
            return cb(err)
          }
