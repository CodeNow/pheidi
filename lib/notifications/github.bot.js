...
function render (gitInfo, instance) {
  ...
  var serverUrl = runnableHostname.direct(opts)
...
function instancePorts (instance) {
  var portsObj = keypather.get(instance, 'containers[0].ports')
  if (!portsObj) {
    return []
  }
...
function defaultPort (instance) {
  var ports = instancePorts(instance)
  if (ports.length > 0) {
...
function createServerLink (instance, link) {
  const url = 'http://' + link + '?' + REF_PR + ':' + defaultPort(instance)
