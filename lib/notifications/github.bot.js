...
class GitHubBot {
  ...
  _deleteComments (gitInfo, cb) {
    ...
    this.github.findCommentsByUser(gitInfo.repo, gitInfo.number,
      ...
      (err, comments) => {
        ...
        this._deleteListOfComments(gitInfo.repo, comments, cb)
      })
  }

  _deleteListOfComments (repo, comments, cb) {
    async.each(comments, (comment, deleteCb) => {
      this.github.deleteComment(repo, comment.id, deleteCb)
    }, cb)
  }
   ...
   * @param {Function} cb - standard callback
   */
  _ensureNoDuplicates (gitInfo, instance, cb) {
    const log = logger.child({
      gitInfo: gitInfo,
    ...
    this.github.findCommentsByUser(gitInfo.repo, gitInfo.number,
      ...
      (err, comments) => {
        ...
        if (instanceComments.length > 1) {
          ...
          this._deleteListOfComments(gitInfo.repo, instanceComments, cb)
        } else {
          return cb()
        }
      })
  ...
  _upsertComment (gitInfo, instance, isolatedInstances, cb) {
    ...
    const done = function (messageId, message) {
      return function (err, resp) {
        ...
        self._ensureNoDuplicates(gitInfo, instance, function (err) {
          cb(err, resp)
        })
      }
    }
    this.github.findCommentsByUser(gitInfo.repo, gitInfo.number,
      ...
      (err, comments) => {
        ...
        this._deleteListOfComments(gitInfo.repo, instanceComments, (err) => {
          if (err) {
