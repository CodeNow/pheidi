...
class GitHubBot {
   ...
   *
   * @return {Boolean} True if runnabot has been enabled for an org
   */
  checkRunnabotEnabledAndAcceptInvite (orgName) {
    const log = logger.child({
      orgName: orgName,
      method: 'checkRunnabotEnabledAndAcceptInvite'
    })
    log.info('checkRunnabotEnabledAndAcceptInvite called')
    return bigPoppa.getOrganizations({ lowerName: orgName.toLowerCase() })
      .get('0')
      .then((org) => {
        if (!org.runnabotEnabled) {
          return this.github.acceptInvitationAsync(orgName)
            .then(function () {
              rabbitmq.publishRunnabotEnabled({
  ...
  notifyOnUpdate (pushInfo, instance, isolatedInstances, cb) {
    ...
    this.checkRunnabotEnabledAndAcceptInvite(instance.owner.username)
      .then(() => {
