...
class GitHubBot {
  ...
  /**
   * Ensure runnabot has no more than 1 comment on the PR page per ICCId
      ...
      (err, comments) => {
        ...
        if (comments.length > 1) {
          ...
          let commentsToDelete = comments.filter((comment) => {
            let parseCommentForICCId = htmlCommentRegEx.exec(comment.body)
            if (parseCommentForICCId) {
              let ICCId = parseCommentForICCId[1]
              if (ICCIds.indexOf(ICCId) !== -1) {
                ICCIds.push(ICCId)
                return false
              }
            }
  ...
  _upsertComment (gitInfo, instance, isolatedInstances, cb) {
    ...
    this.github.findCommentsByUser(gitInfo.repo, gitInfo.number, process.env.RUNNABOT_GITHUB_USERNAME,
      (err, comments) => {
        ...
        const ICCId = instance.inputClusterConfig ? instance.inputClusterConfig._id : process.env.NO_CLUSTER_ID_NAME
        const comment = this._findCommentByICCId(comments, ICCId)
        if (!comment) {
