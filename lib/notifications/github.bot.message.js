...
module.exports = class GitHubBotMessage {
  static render (gitInfo, instance, isolatedInstances) {
    ...
    message += GitHubBotMessage.createMarkdownLink(instance.name, containerUrl)
    message += ' to '
    const instanceUrl = process.env.WEB_URL + '/' + instance.owner.username + '/' + instance.name
    message += GitHubBotMessage.createMarkdownLink('your environment', instanceUrl)
  ...
  static renderIsolatedInstance (isolatedInstances) {
    ...
    if (isolatedInstances && !isEmpty(isolatedInstances)) {
      ...
      const linksList = isolatedInstances.map((instance) => {
        ...
        const link = GitHubBotMessage.createMarkdownLink(readableInstanceName, instanceUrl)
        const statusIcon = GitHubBotMessage.renderStatusIcon(utils.instanceState(instance))
   ...
   * @return {String}
   */
  static createMarkdownLink (title, url) {
    return '[' + title + '](' + url + ')'
  }
}
