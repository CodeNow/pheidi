...
class Slack {
   ...
   * @param  {Object}   instance       deployed instance object
   * @param  {Function} cb
   */
  sendDirectMessage (githubUsername, message, instance, cb) {
    log.trace({
    ...
    const messageId = slackId + '/' + instance._id
    const tracked = tracker.get(messageId)
    if (tracked && deepEqual(tracked, message)) {
  ...
  notifyOnAutoDeploy (gitInfo, githubUsername, instance, cb) {
    ...
    this.sendDirectMessage(githubUsername, message, instance, cb)
    const tags = [
...
function moreChangesSlack (repo, commitLog) {
  ...
  let text = ' and <' + githubMoreLink(repo, commitLog)
